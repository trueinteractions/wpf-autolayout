    ...
    throw (ExCLParseErrorMisc)
{
  ...
  double n1, n2, n3;
  if (_stricmp("required",sz) == 0)
    ; /* initialized to ClsRequired, above */
  else if (_stricmp("strong",sz) == 0) { pcls = &ClsStrong(); }
  else if (_stricmp("medium",sz) == 0) { pcls = &ClsMedium(); }
  else if (_stricmp("weak",sz) == 0) { pcls = &ClsWeak(); }
